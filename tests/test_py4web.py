...
def test_serialize():
    ...
    loaded_data = json.loads(as_json.encode(JsonTable))
    assert loaded_data
    assert isinstance(loaded_data, dict)
    loaded_data = json.loads(as_json.encode(row))
    assert isinstance(loaded_data, dict)
    assert loaded_data

    loaded_data = json.loads(as_json.encode(JsonTable.paginate(limit=1)))

    assert loaded_data["pagination"]
